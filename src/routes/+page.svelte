	...
	<nav class="grid grid-cols-1 sm:grid-cols-3 gap-8">
		{#each data.pages as page}
			<div class="text-center">
				<Link href={`/pages/${page.slug}`} classes="text-xl">
					{page.title}
				</Link>
...
<section>
		...
		{#each data.posts.slice(0, 5) as post}
			<a href={`/posts/${post.slug}`} class="flex flex-col lg:flex-row lg:items-center gap-4 group">
				...
				<div class="flex flex-1 flex-col p-4">
					<h2 class="text-2xl font-bold text-mint-300 group-hover:text-purple-400">
						{post.title}
					</h2>
	...
	<div class="flex justify-center py-8">
		{#if data.posts.length > 5}
			<Link href="/posts" classes="py-3 px-4 rounded-md bg-mint-700/20 hover:bg-purple-700/20">
