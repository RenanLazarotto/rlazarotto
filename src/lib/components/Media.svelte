<!--
    ...
    @param {Types.Media} media Mídia a ser exibida
-->
<script lang="ts">
    ...
    {#if media.type == "image"}
        ...
        >
            <img
                src={media.src}
                alt={media.alt}
                class="max-h-[400px] w-full cursor-pointer rounded object-contain"
            />
            <p class="my-2 text-center text-sm text-gray-400">{media.title}</p>
        </div>
    {:else}
